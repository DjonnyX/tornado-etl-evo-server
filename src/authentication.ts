...
const checkEvoCloudToken = async (token: string, request: express.Request) => {
  return new Promise<void>((resolve, reject) => {
    if (!token) {
      return reject(new ServerError("Token is empty.", ErrorCodes.CLIENT_TOKEN_EMPTY_TOKEN, 401));
    }
    if (token !== config.EVO_CLOUD_TOKEN) {
      return reject(new ServerError("Unauthorized.", ErrorCodes.CLIENT_TOKEN_VERIFICATION, 401));
    }
...
export async function expressAuthentication(
  ...
  if (securityName === "evoCloudAccessToken") {
    ...
    return await checkEvoCloudToken(token, request);
  }
}